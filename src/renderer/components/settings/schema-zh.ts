...
export const ZH_SETTINGS: Record<string, { label: string; description?: string }> = {
	...
	"tui.reactions": {
		label: "Agent 表情回应",
		description: "允许 Agent 在回复气泡上用表情徽章回应你的消息",
	},
	...
	"edit.recoverInlineEdits": {
		label: "恢复内联编辑内容",
		description: "把模型以普通文本输出的编辑内容转换为 edit 工具调用并执行",
	},
	...
	"eval.tools.enabled": {
		label: "Eval 定义的工具",
		description: "允许 eval 单元格定义可供 task、agent() 和 workpool() 子代理调用的工具",
	},
	"eval.workpool.freshAgents": {
		label: "Workpool 使用全新 Agent",
		description: "为每个 workpool 项启动新的子代理，不复用 worker 或批量处理排队项",
	},
	...
	"images.questionTimeoutMs": {
		label: "图片提问超时",
		description: "read 工具通过 ?q= 向视觉模型提问时的单次请求超时（毫秒）。提供商卡顿时快速失败；设为 0 禁用超时。",
	},
	...
	"task.isolation.enabled": {
		label: "隔离子代理",
		description: "在检出的隔离副本中运行子代理，并在完成后整合更改",
	},
	"isolation.backend": {
		label: "隔离后端",
	...
	"worktree.clone": {
		label: "将当前检出克隆到 Worktree",
		description:
			"新 worktree 优先使用写时复制克隆当前检出，使 node_modules、target 等忽略的构建产物一并保留；不支持时回退为普通检出",
	},
	"worktree.cleanSource": {
		label: "创建 /wt 后清理源检出",
		description: "使用 /wt 创建 worktree 并带走更改后，重置源检出的已跟踪更改并删除未跟踪文件",
	},
	...
	enabledProviders: {
		label: "启用的能力提供商",
		description: "额外启用的能力发现提供商 id 列表（支持按路径作用域配置）",
	},
	disabledProviders: {
		label: "禁用的提供商",
